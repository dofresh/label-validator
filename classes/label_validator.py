...
class LabelValidator:
    ...
    def validate_label(self, text, product_category="일반"):
        ...
        snippet = text[:1500]
        prompt = f"""아래 한글 표시사항의 텍스트를 분석하여 오타, 문법 오류, 혹은 논리적 모순(예: 냉동식품에 '18도 이하'와 같이 부적절한 보관 온도)이 있는지 확인하고, 문제가 없으면 "문제 없음"이라고 응답해주는데, 순서는 오류,오타,논리모순을 먼저해주고 구분선을 넣은 다음 수정안을 제시해줘 .
